...
/*   Updated: 2025/06/17 17:30:37 by carfern2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			const char *path, const char *name)
{
	...
	*img_ptr = mlx_xpm_file_to_image(game->mlx, (char *)path, &img_w, &img_h);
	if (!*img_ptr)
	{
		ft_printf("Error\n Could not load %s. Finishing the game.\n", name);
		exit(1);
	}
}
...
void	load_game_assets(t_game *game)
{
	...
	_load_image_asset(game, &game->img_player,
		"images/avatar.xpm", "avatar.xpm");
	_load_image_asset(game, &game->img_collectible,
		"images/fruta.xpm", "fruta.xpm");
